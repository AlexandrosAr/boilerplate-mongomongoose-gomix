...
const findEditThenSave = (personId, done) => {
  ...
  Person.findById(personId, (err, individual) => {
    ...
    individual.markModified('favoriteFoods');
    individual.save((err, data) => {
      if (err) {
        console.error(err);
        return;
      }
...
const findAndUpdate = (personName, done) => {
  ...
  Person.findOneAndUpdate(filter, update, { new: true }, (err, person) => {
    if (err) {
      console.error(err);
      return;
    }
